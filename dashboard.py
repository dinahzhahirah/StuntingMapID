...
def main():
    ...
    with tab3:
        if st.session_state.cluster_results is not None and st.session_state.processed_data is not None:
            ...
            - <strong>AU ≥ 0.95 (95%):</strong> Cluster sangat didukung (highly supported) - sangat reliable, struktur cluster stabil dan tidak terbentuk secara acak <br>
            - <strong>AU 0.90-0.94 (90-94%):</strong> Cluster didukung baik (well supported) - reliable, struktur cluster relatif stabil dan tidak terbentuk secara acak <br>
            - <strong>AU 0.80-0.89 (80-89%):</strong> Cluster didukung moderat (moderately supported) - cukup reliable, struktur cluster menunjukkan pola yang konsisten namun masih memiliki ketidakpastian, kemungkinan terbentuk secara acak relatif rendah tapi tidak dapat diabaikan <br>
            - <strong>AU < 0.80 (80%):</strong> Cluster kurang didukung (poorly supported) - tidak reliable Hasil cluster ini memberikan bukti terhadap hasil pengelompokan sehingga bisa menjadi dasar yang valid untuk aplikasi praktis dalam kebijakan pemerintah. <br>
